...
function myFunction() {
  ...
  client.on('messageCreate', async (message) => {
    if (message.content === 'hello') {
      ...
      // Send the restart message to the parent process
...
function startChildProcess() {
  child = spawn('node', ['-e', `(${myFunction.toString()})();`], { stdio: 'pipe', stdio: ['pipe', 'pipe', 'pipe', 'ipc'] });
...
rl.on('line', (input) => {
  if (input.trim() === 'restart') {
    // Send the restart message to the child process
    child.send('restart');
  }
});
